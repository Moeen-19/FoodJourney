...
import { View, StyleSheet, ScrollView, Pressable, Alert, Share, TextInput } from "react-native";
...
export default function MultiDayItineraryScreen() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const shareUrl = `citywhisper://itinerary/${shareCode}`;
  ...
  const totalBudget = itinerary?.days.reduce((sum, day) => 
    sum + day.stops.reduce((daySum, stop) => daySum + (stop.estimatedCost || 0), 0), 0
  ) || 0;
      ...
      >
        ...
        {showSaved ? (
          <View style={styles.savedSection}>
            <ThemedText type="subtitle" style={styles.sectionTitle}>Saved Itineraries</ThemedText>
          ...
          <>
            <Card elevation={1} style={styles.plannerCard}>
              <ThemedText type="subtitle" style={styles.sectionTitle}>Duration</ThemedText>
              ...
              <ThemedText type="subtitle" style={styles.sectionTitle}>Trip Style</ThemedText>
              <View style={styles.optionsRow}>
              ...
              <Button onPress={handleGenerate} disabled={isLoading} style={{ marginTop: Spacing.lg }}>
              ...
              <>
                <Card elevation={1} style={styles.titleCard}>
                  ...
                  <View style={styles.actionButtons}>
                    <Pressable style={[styles.actionButton, { backgroundColor: theme.primary }]} onPress={handleSave}>
                      <Feather name="save" size={18} color="#fff" />
                    </Pressable>
                    <Pressable style={[styles.actionButton, { backgroundColor: theme.secondary }]} onPress={handleShare}>
                      <Feather name="share-2" size={18} color="#fff" />
                    </Pressable>
                    <Pressable style={[styles.actionButton, { backgroundColor: theme.accent || theme.primary }]} onPress={() => handleExport('ics')}>
                      <Feather name="calendar" size={18} color="#fff" />
                    </Pressable>
                    <Pressable style={[styles.actionButton, { backgroundColor: theme.textSecondary }]} onPress={() => handleExport('text')}>
                  ...
                  {itinerary.days.map((day, index) => (
                    <Pressable
                      ...
                      style={[
                        styles.dayTab,
                        { 
                          backgroundColor: activeDay === day.dayNumber ? theme.primary : theme.cardBackground,
                    ...
                    >
                      <ThemedText 
                        type="subtitle" 
                        style={{ color: activeDay === day.dayNumber ? '#fff' : theme.text }}
                      >
                        Day {day.dayNumber}
                      </ThemedText>
                      <ThemedText 
                        type="caption" 
                        style={{ color: activeDay === day.dayNumber ? '#fff' : theme.textSecondary }}
                      >
