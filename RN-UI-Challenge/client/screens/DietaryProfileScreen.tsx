...
const dietaryOptions: { key: keyof DietaryRestrictions; label: string; icon: string }[] = [
...
const cuisineOptions = [
  ...
  "Mediterranean", "American", "Korean", "Vietnamese", "French", "Middle Eastern"
];

const budgetOptions: { value: "low" | "medium" | "high"; label: string; description: string }[] = [
...
export default function DietaryProfileScreen() {
  ...
  const { profile, saveProfile, completeOnboarding, hasCompletedOnboarding } = useUserProfile();
  
  const isEditing = route.params?.isEditing || hasCompletedOnboarding;
  
  const [name, setName] = useState(profile.name);
  const [city, setCity] = useState(profile.city);
  const [dietary, setDietary] = useState<DietaryRestrictions>(profile.dietaryRestrictions);
  const [budget, setBudget] = useState<"low" | "medium" | "high">(profile.budgetRange);
        ...
        {budgetOptions.map(option => (
          <Pressable
            ...
            style={[
              styles.budgetOption,
              {
                backgroundColor: budget === option.value ? theme.primary + "20" : theme.backgroundDefault,
                borderColor: budget === option.value ? theme.primary : theme.border,
              }
            ]}
          >
