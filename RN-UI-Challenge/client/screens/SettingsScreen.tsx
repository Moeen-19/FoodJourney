...
function SettingItem({ icon, title, subtitle, onPress, destructive }: SettingItemProps) {
  ...
  return (
    ...
    >
      <View style={[styles.settingIcon, { backgroundColor: destructive ? theme.error + "20" : theme.primary + "20" }]}>
        <Feather name={icon} size={20} color={destructive ? theme.error : theme.primary} />
      </View>
      <View style={styles.settingContent}>
        <ThemedText type="body" style={destructive ? { color: theme.error } : undefined}>
          {title}
        </ThemedText>
        {subtitle ? (
      ...
      >
        ...
        <Card elevation={1} style={styles.settingsCard}>
          ...
          <SettingItem
            icon="dollar-sign"
            title="Budget Range"
            subtitle={profile.budgetRange === "low" ? "Budget-Friendly" : profile.budgetRange === "medium" ? "Moderate" : "Flexible"}
            onPress={handleEditProfile}
          />
          <View style={[styles.divider, { backgroundColor: theme.border }]} />
          <SettingItem
        ...
        <Card elevation={1} style={styles.settingsCard}>
          <SettingItem
            ...
            subtitle={profile.preferredCuisines.length > 0 ? profile.preferredCuisines.join(", ") : "None selected"}
            onPress={handleEditProfile}
          />
        </Card>
