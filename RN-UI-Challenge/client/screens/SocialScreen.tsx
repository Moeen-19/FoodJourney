...
import { View, StyleSheet, ScrollView, Pressable, Share, Alert, TextInput } from "react-native";
...
export default function SocialScreen() {
  ...
  const handleShare = async (item: any, type: string) => {
    try {
      const shareUrl = `citywhisper://${type}/${item.shareCode || item.id}`;
      await Share.share({
        title: item.title || item.name,
        message: `Check this out on CityWhisper: ${item.title || item.name}\n\n${shareUrl}`,
      });
    } catch (error) {
  ...
  const handleRemoveFavorite = (businessId: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: "Remove", style: "destructive", onPress: () => removeFavorite(businessId) }
      ]
    );
  };
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      >
        <ThemedText type="h2" style={styles.title}>Social</ThemedText>
                ...
                <Card key={fav.id} elevation={1} style={styles.itemCard}>
                  <View style={styles.itemHeader}>
                    <View style={styles.itemInfo}>
                      <ThemedText type="subtitle">{fav.businessName || "Favorite Spot"}</ThemedText>
                    ...
                    <View style={styles.itemActions}>
                      ...
                      >
                        <Feather name="trash-2" size={16} color={theme.error || '#E76F51'} />
                      </Pressable>
                    </View>
                  </View>
                  {fav.notes && (
        ...
        {activeTab === "friends" && (
          <View style={styles.tabContent}>
            ...
            >
              <Feather name={showAddFriend ? "x" : "user-plus"} size={18} color={theme.primary} />
            ...
            {showAddFriend && (
              <Card elevation={2} style={styles.addFriendCard}>
                <TextInput
                  ...
                  autoCapitalize="none"
                />
                <Button onPress={handleAddFriend} style={{ marginTop: Spacing.md }}>
                  Send Friend Request
                </Button>
              </Card>
            )}
            ...
            {friends.length > 0 ? (
              friends.map((friend) => (
                <Card key={friend.id} elevation={1} style={styles.itemCard}>
                  <View style={styles.friendRow}>
                    ...
                    <Pressable style={[styles.iconButton, { backgroundColor: theme.secondary + '20' }]}>
              ...
              <View style={styles.emptyState}>
                ...
                <ThemedText type="body" style={{ color: theme.textSecondary, textAlign: "center", marginTop: Spacing.md }}>
                  Connect with friends to share your favorite spots and itineraries!
                </ThemedText>
              </View>
            )}
          </View>
        ...
        {activeTab === "shared" && (
          <View style={styles.tabContent}>
            {sharedItems.length > 0 ? (
              ...
              <View style={styles.emptyState}>
                <Feather name="share-2" size={48} color={theme.textSecondary} style={{ opacity: 0.5 }} />
                <ThemedText type="body" style={{ color: theme.textSecondary, textAlign: "center", marginTop: Spacing.md }}>
                  Nothing shared yet. Share your favorites and itineraries with friends!
                </ThemedText>
              </View>
            )}
          </View>
