...
export function useOfflineMode() {
  ...
  const addPendingMutation = useCallback(async (mutation: Omit<PendingMutation, 'id' | 'timestamp'>): Promise<void> => {
  ...
  const syncPendingMutations = useCallback(async (): Promise<void> => {
    ...
    await AsyncStorage.setItem(PENDING_MUTATIONS_KEY, JSON.stringify(failedMutations));
    setIsSyncing(false);
