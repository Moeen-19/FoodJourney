...
export function useSocial() {
  ...
  const addFavorite = useCallback(async (businessId: string, businessName?: string): Promise<Favorite> => {
    const userId = await getUserId();
  ...
  const removeFavorite = useCallback(async (businessId: string): Promise<void> => {
    const userId = await getUserId();
    ...
    const updatedFavorites = favorites.filter(f => f.businessId !== businessId);
    setFavorites(updatedFavorites);
    await AsyncStorage.setItem('favorites', JSON.stringify(updatedFavorites));
  }, [favorites]);

  const addFriend = useCallback(async (friendUsername: string): Promise<void> => {
    const userId = await getUserId();
