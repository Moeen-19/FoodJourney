...
const generateMockItinerary = (duration: number, style: string): MultiDayItinerary => {
...
export function useMultiDayItinerary() {
  ...
  const generateItinerary = useCallback(async (duration: number, style: string) => {
    setIsLoading(true);
  ...
  const saveItinerary = useCallback(async (data: MultiDayItinerary): Promise<MultiDayItinerary> => {
    ...
    try {
      ...
      await AsyncStorage.setItem('savedItineraries', JSON.stringify(savedList));
  ...
  const exportItinerary = useCallback(async (id: string, format: 'ics' | 'text') => {
    try {
  ...
  const shareItinerary = useCallback(async (data: MultiDayItinerary): Promise<string> => {
    if (data.shareCode) {
