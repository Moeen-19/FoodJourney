...
export function useBudgetTracker() {
  ...
  const fetchBudgetData = useCallback(async (days: number = 30) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/budget/${userId}?days=${days}`);
      ...
      if (cached) {
        ...
        const total = cachedData.reduce((sum: number, t: Transaction) => 
          sum + parseFloat(t.amount || '0'), 0);
        const categoryBreakdown: Record<string, number> = {};
        cachedData.forEach((t: Transaction) => {
          categoryBreakdown[t.category] = (categoryBreakdown[t.category] || 0) + parseFloat(t.amount || '0');
        });
  ...
  const addTransaction = useCallback(async (data: {
    ...
    try {
      ...
      await AsyncStorage.setItem('budgetTransactions', JSON.stringify(cachedList));
  ...
  const getSpendingInsights = useCallback(async (days: number = 30): Promise<{
    topCategory: string;
