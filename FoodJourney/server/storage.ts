...
export interface IStorage {
  ...
  publishItinerary(id: string): Promise<Itinerary | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getItineraryByShareCode(code: string): Promise<Itinerary | undefined> {
    const [itinerary] = await db
      .select()
      .from(itineraries)
      .where(eq(itineraries.shareCode, code));
    return itinerary || undefined;
  }
  ...
  async removeFavorite(userId: string, businessId: string): Promise<void> {
    await db
      .delete(favorites)
      .where(
        and(eq(favorites.userId, userId), eq(favorites.businessId, businessId)),
      );
  }
  ...
  async publishItinerary(id: string): Promise<Itinerary | undefined> {
    const [existing] = await db
      .select()
      .from(itineraries)
      .where(eq(itineraries.id, id));
    if (!existing) return undefined;
    const shareCode = existing.shareCode || randomUUID().slice(0, 12);
    const [updated] = await db
      .update(itineraries)
      .set({ isPublic: true, shareCode, updatedAt: new Date() })
      .where(eq(itineraries.id, id))
      .returning();
    return updated;
  }
